  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        sh 'docker run --rm --workdir=/app -v ${WORKSPACE}:/app samepagelabs/zip zip -r moovly-wordpress-plugin dist/* src/* vendor/* moovly.php README.md readme.txt package.json package-lock.json'
